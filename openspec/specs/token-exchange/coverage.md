...
| Ephemeral RSA assertion key generated when the configured key file cannot be read | output | Requirement: Private key resolution |
| 5xx from missing request Authorization, malformed key contents, or missing response `access_token` | output | Requirement: Subject token extraction; Requirement: Private key resolution; Requirement: Successful exchange |
