...
const PREFIX = uniquePrefix("mtls-acl-allow");
const ALICE_CN = certCommonName("alice"); // "Alice Example"

test.describe("mtls-acl — allow-list evaluation", () => {
  ...
  // [Verifies: mtls-acl.allow-list-evaluation.match-grants-access]
  test("attribute value matching an allow-list entry grants access", async ({
    request,
  }) => {
  ...
  // [Verifies: mtls-acl.allow-list-evaluation.no-match-rejected]
  test("attribute value not in the allow list is rejected", async ({
    request,
  }) => {
    const { routePath } = await provisionPluginRoute(request, {
      prefix: PREFIX,
      config: {
        certificate_attribute: "common_name",
        allow: ["Bob Example", "Another Client"],
      },
    });
  ...
  // [Verifies: mtls-acl.allow-list-evaluation.case-only-mismatch-rejected]
  test("case-only mismatch against the allow list is rejected", async ({
    request,
  }) => {
