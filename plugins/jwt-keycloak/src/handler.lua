...
local function custom_validate_token_signature(conf, jwt, second_call)
  ...
  local public_keys,
    err = kong.cache:get(issuer_cache_key, nil, custom_helper_issuer_get_keys, well_known_endpoint, conf.cafile)
...
local function retrieve_tokens(conf)
  ...
  for _, v in ipairs(conf.header_names) do
    ...
    if token_header then
      ...
      local iterator,
        iter_err = re_gmatch(token_header, "\\s*[Bb]earer\\s+(.+)")
...
local function do_authentication(conf)
  ...
  local ok_claims,
    errors = jwt:verify_registered_claims(conf.claims_to_verify)
  ...
  if conf.maximum_expiration ~= nil and conf.maximum_expiration > 0 then
    local ok,
      max_exp_errors = jwt:check_maximum_expiration(conf.maximum_expiration)
    if not ok then
      return false, {status = 403, message = "Token claims invalid: " .. custom_helper_table_to_string(max_exp_errors)}
    end
  end
  ...
  if conf.consumer_match then
    local ok,
      err = custom_match_consumer(conf, jwt)
    if not ok then
      return ok, err
    end
  end
  ...
  local ok,
    err = validate_scope(conf.scope, jwt.claims)

  if ok then
    ok,
      err = validate_realm_roles(conf.realm_roles, jwt.claims)
  end

  if ok then
    ok,
      err = validate_roles(conf.roles, jwt.claims)
  end

  if ok then
    ok,
      err = validate_client_roles(conf.client_roles, jwt.claims)
  end
