...
local function load_consumer_by_username(username)
  local result,
    err = kong.db.consumers:select_by_username(username)
  if not result then
    return nil, err
  end
...
local function match_consumer(consumer_id)
  ...
  consumer,
    err = kong.cache:get(consumer_cache_key, nil, load_consumer_by_username, consumer_id)
