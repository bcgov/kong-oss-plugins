...
local function verify_bearer_jwt(oidcConfig)
  ...
  local discovery_doc,
    err = require("resty.openidc").get_discovery_doc(opts)
  if err then
  ...
  local json,
    err,
    _ = require("resty.openidc").bearer_jwt_verify(opts, claim_spec)
  if err then
...
local function introspect(oidcConfig)
  if utils.has_bearer_access_token() or oidcConfig.bearer_only == "yes" then
    ...
    if oidcConfig.use_jwks == "yes" then
      res,
        err = require("resty.openidc").bearer_jwt_verify(oidcConfig)
    else
      res,
        err = require("resty.openidc").introspect(oidcConfig)
    end
    if err then
  ...
  if response == nil then
    ...
    if response then
      ...
      if (not oidcConfig.disable_access_token_header and response.access_token) then
        utils.injectAccessToken(
          response.access_token,
          oidcConfig.access_token_header_name,
