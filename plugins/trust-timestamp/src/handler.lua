...
function create_timestamp_query(data)
  ...
  -- MessageImprint
  local message_imprint = der_sequence(algorithm_id, der_octet_string(hash))
  ...
  local tsq_der = der_sequence(version, message_imprint, nonce, cert_req)
...
local function call_ts_authority(tsa_url, policy_oid, artifact)
  local tsq_der,
    nonce = create_timestamp_query(artifact)
  local response_body = {}
  local res,
    status_code,
    headers,
